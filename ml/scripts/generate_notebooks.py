...
NOTEBOOKS: dict[str, list[tuple[str, str]]] = {
    "01_data_collection": [
        ...
        (
            "markdown",
            "Re-run `snapshot_all()` any time the underlying tables change — every notebook after this one reads the parquet snapshot, not a live query, so results stay consistent within one analysis session.",
        ),
    ],
    "02_data_cleaning": [
        (
    ...
    "05_skill_analysis": [
        ...
        (
            "markdown",
            "See `ml/models/skill_clustering/README.md` for the saved model card (features, baseline, honest limitations at this taxonomy's current embedding coverage).",
        ),
    ],
    "06_job_analysis": [
        (
        ...
        (
            "code",
            "from training.job_category import train as train_job_category\n\ntrain_job_category()",
        ),
    ...
    "07_model_training": [
        ...
        (
            "code",
            "from training.suitability import train as train_suitability\n\ntrain_suitability()",
        ),
        (
            "code",
            "from training.career_rank import train as train_career_rank\n\ntrain_career_rank()",
        ),
        (
            "code",
            "from training.skill_demand import train as train_skill_demand\n\ntrain_skill_demand()",
        ),
    ],
    "08_model_evaluation": [
        (
